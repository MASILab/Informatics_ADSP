...
do
    ...
    #now, run freesurfer
    time singularity exec -e --contain -B $license:/usr/local/freesurfer/.license -B $line:$line -B $out_dir:$out_dir $simg_path recon-all -i $line -subjid freesurfer -sd $out_dir/ -all > $out_dir/freesurfer.log
